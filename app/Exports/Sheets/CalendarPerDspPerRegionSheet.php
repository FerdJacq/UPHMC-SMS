...
class CalendarPerDspPerRegionSheet implements FromCollection, WithEvents, WithMapping ,  WithHeadings,  ShouldAutoSize , WithStrictNullComparison , WithTitle , WithStyles , WithCustomStartCell , WithColumnWidths 
{
    ...
    {   
        $data=$this->data["summaries"]->groupBy(function ($item, $key) {
            return $item->company_name."-".$item->region_name;
        });
        $data = collect($data->mapWithKeys(function ($item,$key) {
            return  [
                $key =>[
                    ...
                    'region_name' => $item->first()->region_name,
                    'region_code' => $item->first()->region_code,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                if(!$this->data["seller_id"]) $event->sheet->getColumnDimension('D')->setVisible(false);
