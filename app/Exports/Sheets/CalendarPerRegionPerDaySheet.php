...
class CalendarPerRegionPerDaySheet implements FromCollection, WithEvents, WithMapping ,  WithHeadings,  ShouldAutoSize , WithStrictNullComparison , WithTitle , WithStyles , WithCustomStartCell , WithColumnWidths 
{
    ...
    {   
        $data=$this->data["summaries"]->groupBy(function ($item, $key) {
            return $item->company_name."-".$item->assigned_date."-".$item->region_name;
        });
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $event->sheet->getDelegate()
                ->getStyle("C" . $first_row . ':' . "C" . $computation_row)
                ->getNumberFormat()
