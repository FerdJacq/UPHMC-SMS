<template>
              ...
              <div class="card h-100">
                <div class="card-body">
                  ...
                  <span class="p-float-label mt-3">
                    <p-dropdown
                        v-model="selected_transaction_types"
                  ...
                  <span class="p-float-label mt-3" v-if="!['SELLER'].includes(_role)">
                    <p-dropdown
                        v-model="selected_seller_type"
                        :options="seller_type"
                        optionLabel="name"
                        optionValue="val"
                        class="w-full"
                  ...
                  <span class="p-float-label mt-3" v-if="!['SELLER'].includes(_role)">
                    <p-multiselect
                      ...
                      optionValue="val"
                      display="chip"
                      v-model="selected_eligible_witheld_seller"
                      inputClass="form-control shadow-none"
                    />
                    <label for="company_name">Eligible Withholding Tax</label>
                  </span>
              ...
              <table class="table table-bordered table-fit-column-1 table-sm">
                <thead>
                  <tr>
                    ...
                    <td class="w-fit">Total Tax Due</td>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, index) in dialog.data" :key="index" :class="{'d-none':!item.total_tax>0}">
                    <td v-if="dialog.type">
                      <div class="avatar-sm">
                        <img :src="item.logo" class="img-fluid rounded-circle"/>
                      </div>
  ...
  export default {
    ...
    computed: {
      service_providers(){
        return this.getDataPerCompany().sort((a, b) => b.total_tax - a.total_tax);
      }
    },
    data() {
      return {
        ...
        region_list:[],
        selected_regions:[],
        selected_transaction_types:"all",
        selected_seller_type:"all",
        seller_type:[
          {name:"All",val:"all"},
          {name:"Non Individual",val:"CORP"},
          {name:"Individual",val:"INDIVIDUAL"}
        ],
        selected_eligible_witheld_seller:["NONE","ELIGIBLE","ACTIVE"],
    ...
    methods:{
        updateData(loading=true) {
          ...
          let params = {
            ...
            transaction_type:this.selected_transaction_types,
            seller_type:this.selected_seller_type,
            eligible_witheld_seller:this.selected_eligible_witheld_seller
        ...
        getDataPerCompany(filtered_date="",per_region=false,company_name=""){
          ...
          .reduce((parent, item) => {
            ...
            parent[group] = parent[group] || {total:0,tax:0,online_platform_vat:0,shipping_vat:0,item_vat:0,transaction_fee:0,service_fee:0,commission_fee:0,withholding_tax:0};
        ...
        computeDialog(type,decimal=2){
          if (type=="total_transaction"){
            return this.formatNumber(this.dialog.data.filter(e=>e.total_tax>0).map(e=>{return e[type] }).reduce((a, b) => a + b, 0),decimal);
          }
    ...
    watch: {
      ...
      "selected_transaction_types":function (val) {
        this.updateData();
      },
      "selected_seller_type":function (val) {
        this.updateData();
      },
      "selected_eligible_witheld_seller":function (val) {
        this.updateData();
      },
    }
  };
